...
class TaskModel:
    try:
        ...
        def getTransaction(self):
            sql=f"SELECT * FROM dbo.hourly_sales WHERE POS_AGENT_MAPPING_HEADER IN (SELECT TOP ({bulklimit}) HEADER_ID FROM dbo.header_sales mh WHERE mh.TAG=0 AND YEAR(mh.TRN_DATE)=YEAR(GETDATE()) ORDER BY mh.TRN_DATE ASC);"
            return db.fetchAll(sql)
        # def getDaily(self):
