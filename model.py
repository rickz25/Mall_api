...
class TaskModel:
    ...
    def getMappingHeader(self):
        try:
            sql=f"SELECT TOP ({bulklimit}) HEADER_ID,TEMPLATE,TRN_DATE,MERCHANT_CODE,POS_VENDOR_CODE,CREATED_AT,LAST_MODIFIED,SUBMISSION_FLAG,MALL_CODE,TAG FROM dbo.header_sales mh WITH (NOLOCK) WHERE mh.TAG=0 AND YEAR(TRN_DATE)=YEAR(GETDATE()) ORDER BY mh.TRN_DATE ASC;"
            return db.fetchAll(sql)
        except Exception as e:
            logger.exception("Exception occurred when select header_sales: %s", str(e))
    def getTransaction(self):
        try:
            sql=f"SELECT * FROM dbo.hourly_sales WITH (NOLOCK) WHERE POS_AGENT_MAPPING_HEADER IN (SELECT TOP ({bulklimit}) HEADER_ID FROM dbo.header_sales mh WITH (NOLOCK) WHERE mh.TAG=0 AND YEAR(mh.TRN_DATE)=YEAR(GETDATE()) ORDER BY mh.TRN_DATE ASC);"
            return db.fetchAll(sql)
        except Exception as e:
    ...
    def getMappingLogs(self):
        try:
            sql=f"SELECT TOP ({bulklimit}) STATUS, MESSAGE, TEXT, TEMPLATE, TRN_DATE, FILENAME, MERCHANT_CODE, STORE_NAME, MALL_CODE, TERMINAL_NO, IS_NULL_ERROR, VALIDATED_AT, TAG FROM dbo.mapping_log WITH (NOLOCK) WHERE STATUS=0 AND TAG=0 AND YEAR(TRN_DATE)=YEAR(GETDATE()) ORDER BY TRN_DATE ASC;"
            return db.fetchAll(sql)
        except Exception as e:
